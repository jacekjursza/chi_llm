...
class TestConfigLoading:
    ...
    def test_provider_config_from_file(self):
        """Test loading provider configuration from a YAML file."""
        cfg = {
            "provider": {
                "type": "local",
                "model": "qwen3-1.7b",
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            ...
            assert config["provider"]["model"] == "qwen3-1.7b"
            assert config["provider"]["host"] == "localhost"
            assert config["provider"]["port"] == 11434
    ...
    @patch.dict(
        "os.environ",
        {
            "CHI_LLM_PROVIDER_TYPE": "local",
            "CHI_LLM_PROVIDER_MODEL": "gemma-270m",
            "CHI_LLM_PROVIDER_HOST": "127.0.0.1",
            "CHI_LLM_PROVIDER_PORT": "11434",
            "CHI_LLM_PROVIDER_API_KEY": "secret",
        },
    )
    def test_provider_config_from_env(self):
        """Test provider configuration loaded from environment variables."""
        config = load_config()
        assert config["provider"]["type"] == "local"
        assert config["provider"]["model"] == "gemma-270m"
        assert config["provider"]["host"] == "127.0.0.1"
        assert config["provider"]["port"] == 11434
